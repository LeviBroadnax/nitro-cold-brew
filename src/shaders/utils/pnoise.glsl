...
float pnoise(in vec4 P, in vec4 rep) {
  ...
  vec4 Pi1 = mod(Pi0 + 1.0f, rep); // Integer part + 1 mod rep
  Pi0 = mod289(Pi0);
  Pi1 = mod289(Pi1);
  ...
  vec4 gx00 = ixy00 * (1.0f / 7.0f);
  vec4 gy00 = floor(gx00) * (1.0f / 7.0f);
  vec4 gz00 = floor(gy00) * (1.0f / 6.0f);
  gx00 = fract(gx00) - 0.5f;
  gy00 = fract(gy00) - 0.5f;
  gz00 = fract(gz00) - 0.5f;
  vec4 gw00 = vec4(0.75f) - abs(gx00) - abs(gy00) - abs(gz00);
  vec4 sw00 = step(gw00, vec4(0.0f));
  gx00 -= sw00 * (step(0.0f, gx00) - 0.5f);
  gy00 -= sw00 * (step(0.0f, gy00) - 0.5f);

  vec4 gx01 = ixy01 * (1.0f / 7.0f);
  vec4 gy01 = floor(gx01) * (1.0f / 7.0f);
  vec4 gz01 = floor(gy01) * (1.0f / 6.0f);
  gx01 = fract(gx01) - 0.5f;
  gy01 = fract(gy01) - 0.5f;
  gz01 = fract(gz01) - 0.5f;
  vec4 gw01 = vec4(0.75f) - abs(gx01) - abs(gy01) - abs(gz01);
  vec4 sw01 = step(gw01, vec4(0.0f));
  gx01 -= sw01 * (step(0.0f, gx01) - 0.5f);
  gy01 -= sw01 * (step(0.0f, gy01) - 0.5f);

  vec4 gx10 = ixy10 * (1.0f / 7.0f);
  vec4 gy10 = floor(gx10) * (1.0f / 7.0f);
  vec4 gz10 = floor(gy10) * (1.0f / 6.0f);
  gx10 = fract(gx10) - 0.5f;
  gy10 = fract(gy10) - 0.5f;
  gz10 = fract(gz10) - 0.5f;
  vec4 gw10 = vec4(0.75f) - abs(gx10) - abs(gy10) - abs(gz10);
  vec4 sw10 = step(gw10, vec4(0.0f));
  gx10 -= sw10 * (step(0.0f, gx10) - 0.5f);
  gy10 -= sw10 * (step(0.0f, gy10) - 0.5f);

  vec4 gx11 = ixy11 * (1.0f / 7.0f);
  vec4 gy11 = floor(gx11) * (1.0f / 7.0f);
  vec4 gz11 = floor(gy11) * (1.0f / 6.0f);
  gx11 = fract(gx11) - 0.5f;
  gy11 = fract(gy11) - 0.5f;
  gz11 = fract(gz11) - 0.5f;
  vec4 gw11 = vec4(0.75f) - abs(gx11) - abs(gy11) - abs(gz11);
  vec4 sw11 = step(gw11, vec4(0.0f));
